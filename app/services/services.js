...
app.service('Api', ['$http', '$q', '$timeout', '$location', 'LocalStorage', function ($http, $q, $timeout, $location, storage) {

    var _this = this;

    function onNotAllowed(deferred, error) {
        deferred.reject(error);
        $location.$$lastRequestedPath = $location.path(); // $route.current.$$route.originalPath;
        $location.path('/signin');        
    }
    function onError(deferred, method, url, response) {
        ...
        if (error.message === 'ERROR_NOT_ALLOWED') {
            ...
            if (q) {
                _this.auth.sso(q).then(function (user) {
                    console.log('Api.auth.sso.success', user);
                    _this[method](url).then(function (response) {
                        console.log(method, url, 'success', response);
                        deferred.resolve(response.data);
                    }, function (response) {
                        console.log(method, url, 'error', response);
                        var error = response ? { message: response.data ? response.data.message : "error", status: response.status } : null;
                        deferred.reject(error);
                    })
                }, function (response) {
                    console.log('Api.auth.sso.error', response);
                    onNotAllowed(deferred, error);
                });
            } else {
                onNotAllowed(deferred, error);
            }
    ...
    var _get = this.get = function (url, params) {
        ...
        $http.get(url, { params: params }).then(function (response) {
            ...
            onError(deferred, 'get', url, response);
        });
    ...
    var _post = this.post = function (url, model) {
        ...
        if (_this.DEBUG) {
            ...
            $http.post(url, model).then(function (response) {
                ...
                onError(deferred, 'post', url, response);
            });
        }
        return deferred.promise;
    };
    var _put = this.put = function (url, model) {
        ...
        $http.put(url, model).then(function (response) {
            ...
            onError(deferred, 'put', url, response);
        });
        return deferred.promise;
    };
    var _patch = this.patch = function (url, model) {
        ...
        $http.patch(url, model).then(function (response) {
            ...
            onError(deferred, 'patch', url, response);
        });
        return deferred.promise;
    };
    var _delete = this.delete = function (url) {
        ...
        $http.delete(url).then(function (response) {
            ...
            onError(deferred, 'delete', url, response);
        });
        return deferred.promise;
    };
...
app.service('Appearance', ['colTypes', 'taskStatus', 'teamStatus', 'formStatus', 'negotiationStatus', 'estimationStatus', 'userGroups', function (colTypes, taskStatus, teamStatus, formStatus, negotiationStatus, estimationStatus, userGroups) {

    this.taskClass = function (statusId) {
        /*
        STATUS_OPENED:     1,
        STATUS_HANDLED:    2,
        STATUS_WAITING:    3,
        ...
        */
        var color = 'idle';
        switch (statusId) {
            case taskStatus.STATUS_OPENED:
                color = 'red';
                break;
            case taskStatus.STATUS_HANDLED:
                color = 'light-green';
                break;
            ...
            case taskStatus.STATUS_COMPLETED:
                color = 'green';
                break;
            case taskStatus.STATUS_ESTIMATION:
                color = 'orange'; // ?
                break;
            case taskStatus.STATUS_PLANNED:
                color = 'light-blue'; // ?
                break;
        }
        return color;
    }
    this.teamClass = function (statusId) {
        /*
        STATUS_UNDONE:      1,
        STATUS_DONE:        2,
        */
        var color = 'idle';
        switch (statusId) {
            case teamStatus.STATUS_UNDONE:
                color = 'red';
                break;
            case teamStatus.STATUS_DONE:
                color = 'green';
                break;
        }
        return color;
    }
    this.projectClass = function (statusId) {
        /*
        STATUS_OPENED:     1,
        STATUS_HANDLED:    2,
        STATUS_WAITING:    3,
    ...
    this.grcClass = function (statusId) {
        /*
        STATUS_WAITING:  1,
        STATUS_TASK:     2,
        STATUS_COM:      3,
        ...
        */
        var color = 'idle';
        switch (statusId) {
            case formStatus.STATUS_WAITING:
                color = 'light-orange';
                break;
            case formStatus.STATUS_TASK:
                color = 'red';
                break;
            case formStatus.STATUS_COM:
                color = 'purple';
                break;
            case formStatus.STATUS_BUDGET:
                color = 'light-blue';
                break;
            case formStatus.STATUS_COMPLETE:
                color = 'green';
                break;
        }
        return color;
    }
    this.negotiationClass = function (statusId) {
        /*
        STATUS_OPENED: 1,
        STATUS_CLOSED: 2,
        */
        var color = 'idle';
        switch (statusId) {
            case negotiationStatus.STATUS_OPENED:
                color = 'red'
                break;
            case negotiationStatus.STATUS_CLOSED:
                color = 'green'
                break;
        }
        return color;
    }
    this.estimationClass = function (statusId) {
        /*
        STATUS_RAI:         255,
        STATUS_COM:         270,
        ...
        */
        var color = 'idle';
        switch (statusId) {
            case estimationStatus.STATUS_RAI:
                color = 'light-orange';
                break;
            case estimationStatus.STATUS_COM:
                color = 'purple';
                break;
            case estimationStatus.STATUS_NEGOTIATION:
                color = 'light-green';
                break;
            case estimationStatus.STATUS_ACCEPTED:
                color = 'green';
                break;
            case estimationStatus.STATUS_REFUSED:
                color = 'red';
                break;
            case estimationStatus.STATUS_COMPLETED:
                color = 'light-blue';
                break;
        }
        return color;
    ...
    this.roleClass = function (roleId) {
        var color = 'idle', role = null;
        angular.forEach(userGroups, function (roles, key) {
            if (!role && roles.indexOf(roleId) !== -1) {
                role = key;
            }
        });
        switch (role) {
...
app.service('Links', ['$timeout', '$location', 'domain', function ($timeout, $location, domain) {
    this.customerRequest = function (id, target) {
        return { href: domain + '/richieste-clienti/' + id, target: target || '_self' };
    };
    this.presale = function (id, target) {
        return { href: '//project.websolute.it/DB_TabelleScheda.asp?tbl=ws_progetti&id=' + id, target: target || '_blank' };   
    };
    this.negotiation = function (id, target) {
        return { href: domain + '/trattative/' + id, target: target || '_self' };
    };
    this.macro = function (macroId, negotiationId, target) {
        return { href: domain + '/trattative/' + negotiationId + '/macrostime/' + macroId, target: target || '_self' };
    };
    this.estimation = function (estimationId, target) {
        return { href: '//project.websolute.it/preventivi_scheda.asp?id=' + estimationId, target: target || '_blank' };
    };
    this.negotiationRecord = function (estimation, negotiation, target) {
        if (estimation.type === 2) {
            return this.macro(estimation.id, negotiation.id, target);
        } else {
            return this.estimation(estimation.id, target);
        }
    };
    this.macroCopy = function (item, target) {
        return { href: domain + '/trattative/' + item.negotiation.id + '/macrostime/' + item.id + '/duplica', target: target || '_self' };       
    };
    this.task = function (id, target) {
        if (id) {
            return { href: '//project.websolute.it/zz_FAF_report_richieste_cliente_detail_2015.asp?RC_ID=' + id, target: target || '_blank' };
        } else {
            return { href: '//project.websolute.it/richiesta_cliente.asp?cl_id=47', target: target || '_blank' };
        }
    };
    this.customer = function (customerId, target) {
        return { href: '//project.websolute.it/DB_TabelleScheda.asp?tbl=ws_clienti&id=' + customerId, target: target || '_blank' };
    };
    this.projectCustomer = function (projectId, customerId, target) {
        return { href: domain + '/clienti/' + customerId + '/progetti/' + projectId, target: target || '_self' };
    };
    this.taskProject = function (taskId, projectId, customerId, target) {
        return { href: domain + '/clienti/' + customerId + '/progetti/' + projectId + '/tasks/' + taskId, target: target || '_self' };
    };
    this.teamTask = function (teamId, taskId, projectId, customerId, target) {
        return { href: domain + '/clienti/' + customerId + '/progetti/' + projectId + '/tasks/' + taskId + '/team/' + teamId, target: target || '_self' };
    };
    this.order = function (id, target) {
        return { href: '//project.websolute.it/DB_TabelleScheda.asp?tbl=ws_ordini&id=' + id, target: target || '_blank' };
    };
    this.project = function (id, target) {
        return { href: '//project.websolute.it/DB_TabelleScheda.asp?tbl=ws_progetti&id=' + id, target: target || '_blank' };
    };
...
app.factory('CustomersAutocomplete', ['$q', '$timeout', 'Api', function ($q, $timeout, Api) {
    var LIMIT = 20;
    function CustomersAutocomplete() {
    }
    CustomersAutocomplete.prototype = {
        index: 0,
        setPhrase: function (phrase) {
            ...
            var deferred = $q.defer();
            Api.prospects.autocomplete(phrase, LIMIT).then(function success(response) {
                var data = {
                    items: [],
        ...
        setItem: function (item) {
            var deferred = $q.defer();
            Api.prospects.detail(item.id).then(function success(response) {
                // console.log('autoComplete.setItem', response);
                deferred.resolve(response);
...
app.factory('DataSource', ['$q', '$http', '$rootScope', '$location', 'LocalStorage', 'Api', 'State', 'DataFilter', function ($q, $http, $rootScope, $location, storage, Api, State, DataFilter) {
    var PAGES_FIRST = 1;
    var PAGES_SIZE = 7;
    ...
    DataSource.prototype = {
        ...
        error: function (response) {
            ...
            if (error.message === 'ERROR_NOT_ALLOWED') {
                ...
                console.log('onError', error, 'q', q);
                if (q) {
                    Api.auth.sso(q).then(function (user) {
                        console.log('Api.auth.sso.success', user);
                        this.get();
                    }.bind(this), function (response) {
                        console.log('Api.auth.sso.error', response);
